...
- `analyzer2.py`
  Main CLI. Handles argument parsing, Gemini upload/cache logic, prompt construction, response parsing, and result archival.
- `pdf_tools.py`
...
- `mit_analyzer.py`
  Older analyzer implementation retained for comparison/reference.
...
```bash
uv run python analyzer2.py path/to/file.pdf
uv run python analyzer2.py file1.pdf file2.pdf file3.pdf
```
...
- PDF preparation layer
  This is local-only logic in `pdf_tools.py`. It decides what bytes should be sent to Gemini.
- Gemini analysis layer
  This is the network-facing logic in `analyzer2.py`. It uploads the prepared files, builds the prompt, requests structured output, and archives the result.
...
- the application avoids building a second-stage merge/rerank pipeline
...
- the top embedded images by size/DPI

This report is diagnostic. It is printed for oversized PDFs so we can understand whether the file is scan-heavy, image-heavy, or structurally bloated.
...
The tool writes two persistent outputs:

- `cost.csv`
  Prompt, candidate, and total token counts per run.
- `output.jsonl`
  Structured analysis records, one JSON object per processed input PDF.
...
The local PDF work is reusable and testable on its own. That is why `pdf_tools.py` and `pdf_compress.py` exist.
...
- Generated chunk files are written next to the source PDF and are not automatically cleaned up.
...
- automatic cleanup of generated chunk files
